...
export default function Auth() {
  ...
  const [signupRole, setSignupRole] = useState("acheteur");
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const fullWhatsapp = signupRole === "vendeur" ? `${whatsappCode}${whatsappNumber}` : undefined;
      await signUp(signupEmail, signupPassword, signupName, signupRole, fullWhatsapp);
      
      if (signupRole === "vendeur") {
        localStorage.setItem("intended_role", "seller");
      }
              ...
              <TabsContent value="signup" className="mt-0">
                <form onSubmit={handleSignup} className="space-y-4">
                  <div className="space-y-3 pb-2 animate-in fade-in slide-in-from-bottom-2 duration-500">
                    ...
                    <RadioGroup defaultValue="acheteur" value={signupRole} onValueChange={setSignupRole} className="grid grid-cols-2 gap-4">
                      <div className="relative">
                        <RadioGroupItem value="acheteur" id="role-acheteur" className="peer sr-only" />
                        <Label
                          htmlFor="role-acheteur"
                          className="flex flex-col items-center justify-between rounded-xl border-2 border-muted bg-popover p-4 hover:bg-accent/5 peer-data-[state=checked]:border-accent peer-data-[state=checked]:bg-accent/5 [&:has([data-state=checked])]:border-accent cursor-pointer transition-all duration-300 group"
                        >
                          <User className="mb-2 h-8 w-8 text-muted-foreground group-hover:scale-110 transition-transform peer-data-[state=checked]:text-accent" />
                          <span className="font-bold">Acheteur</span>
                          <span className="text-[10px] text-muted-foreground text-center mt-1">Pour shopper sereinement</span>
                        </Label>
                      </div>
                      <div className="relative">
                        ...
                        <Label
                          ...
                          className="flex flex-col items-center justify-between rounded-xl border-2 border-muted bg-popover p-4 hover:bg-accent/5 peer-data-[state=checked]:border-accent peer-data-[state=checked]:bg-accent/5 [&:has([data-state=checked])]:border-accent cursor-pointer transition-all duration-300 group"
                        >
                          <Store className="mb-2 h-8 w-8 text-muted-foreground group-hover:scale-110 transition-transform peer-data-[state=checked]:text-accent" />
                          <span className="font-bold">Vendeur</span>
                          <span className="text-[10px] text-muted-foreground text-center mt-1">Pour vendre et booster</span>
                        </Label>
                      </div>
                    </RadioGroup>
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="signup-name">Nom complet / Nom Boutique</Label>
                    <div className="relative">
                      <User className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                      <Input id="signup-name" placeholder="Votre nom" className="pl-9" value={signupName} onChange={e => setSignupName(e.target.value)} required />
                    </div>
                  </div>
                  <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Input
                            ...
                            required={signupRole === "vendeur"}
                          />
                        </div>
                        <p className="text-[10px] text-muted-foreground italic">Format international sans le code pays.</p>
                      </div>
                    </>
                  )}
                  <Button type="submit" className="w-full bg-accent text-accent-foreground hover:bg-accent/90" disabled={isLoading}>
